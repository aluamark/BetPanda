...
import useRunningMatchesQuery from "@/utils/react-query/useRunningMatchesQuery";
...
const RunningMatches = () => {
	...
	const {
		...
		isFetchingNextPage,
		status,
	} = useRunningMatchesQuery(game);

	return status === "loading" ? (
		<MatchesSkeleton />
	) : status === "error" ? (
		<div className="bg-zinc-900 rounded-lg">
			<h2 className="p-3 md:p-5">Running Matches</h2>
							...
							{page.length !== 0 ? (
								page.map((match) => {
									if (match.opponents.length === 2)
										return <RunningMatch key={match.id} match={match} />;
								})
							) : (
