...
import useUpcomingMatchesQuery from "@/utils/react-query/useUpcomingMatchesQuery";
...
const UpcomingMatches = () => {
	...
	const {
		...
		isFetchingNextPage,
		status,
	} = useUpcomingMatchesQuery(game);

	return status === "loading" ? (
		<MatchesSkeleton />
	) : status === "error" ? (
		<div className="bg-zinc-900 rounded-lg">
			<h2 className="p-3 md:p-5">Upcoming Matches</h2>
							...
							{page.length !== 0 ? (
								page.map((match) => {
									if (match.opponents.length === 2)
										return <UpcomingMatch key={match.id} match={match} />;
								})
							) : (
